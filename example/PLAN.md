...
- 2026-07-09 —— Phase 2 复现爬梯第 3–5 级(#8):数据集下载完成(MD5 校验通过)。第 3 级实跑时发现并修复一处 macOS 专属 bug(`DataLoader num_workers=2` 在 spawn 起动方式下递归死锁,改为非 CUDA 设备 `num_workers=0`);修复后 5 epoch 短训练评测(EXP-02)、20 epoch 小规模训练(EXP-03)均通过,test acc 分别到 65.08%/84.92%,loss 单调下降、checkpoint/日志落位正常。第 5 级完整 200 epoch 训练启动后台跑到第 37 个 epoch(best test acc 86.49%),作者本人决定提前终止——验证跑的目的是走通爬梯本身而非逼近论文精度(ADR 0001),故不再等 200 epoch 收敛,[EXP-04](experiments/EXP-04-phase2-repro-full-training.md) 记录了这一范围收窄决定。issue #8 的复现爬梯六级已全部走通并有对应记录;完整训练精度差距对比这一条原始验收标准因上述决定未满足,已在 issue 收尾时说明。
